            ...
            <div className={cn(
              ...
              <div className="relative aspect-video rounded-2xl overflow-hidden card-shadow group/video bg-black cursor-pointer"
                ...
                {product.badge && !isPlaying && (
                  <div className={cn(
                    "absolute top-6 left-6 z-20 flex items-center gap-2 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg",
                    getBadgeStyles()
                  )}>
                    {getBadgeIcon()}
                    {product.badge}
                ...
                {!isPlaying ? (
                  <>
                    <img 
                      ...
                      alt={product.title} 
                      className={cn(
                        "w-full h-full object-cover transition-transform duration-700 group-hover/video:scale-110",
                        isOutOfStock ? "grayscale opacity-50" : "opacity-80"
                      )} 
                    />
                    {!isOutOfStock && product.videoUrl && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <div className="w-20 h-20 bg-white/20 backdrop-blur-xl border border-white/30 rounded-full flex items-center justify-center animate-pulse group-hover/video:scale-110 transition-transform">
                  ...
                  <div className="w-full h-full relative" onClick={toggleVideoPlay}>
                    <video 
                      ...
                      onError={(e) => {
                        ...
                        setIsPlaying(false);
                        // Optionally show an alert or placeholder
                      }}
                    />
                    
                    {/* Controls Overlay */}
                    <div className="absolute inset-0 z-30 opacity-0 group-hover/video:opacity-100 transition-opacity flex flex-col justify-between p-6 bg-gradient-to-t from-black/60 via-transparent to-black/30 pointer-events-none">
                    ...
                    {isPaused && (
                      <div className="absolute inset-0 flex items-center justify-center bg-black/10 pointer-events-none z-20">
                        <div className="w-20 h-20 bg-white/20 backdrop-blur-2xl border border-white/20 rounded-full flex items-center justify-center">
                          <Play size={32} className="text-white fill-white ml-1" />
                        </div>
                      </div>
                    )}
